...
**Kubernetes (2026-08-15) — added at the developer's explicit request; turns out K8s/Docker hands-on learning was the actual primary goal of this whole project, not just the AI-agent portfolio narrative.**
* `k8s/` — raw YAML (no Helm, deliberately, for learning the primitives), targeting **minikube**: `namespace.yaml`, `configmap.yaml` (non-secret env), `secret.example.yaml` (template — copy to `secret.yaml`, gitignored, never commit the real one), `postgres.yaml` / `ollama.yaml` / `agent.yaml` (each a PVC + Deployment + Service). Full setup steps and a "what's deliberately simplified as a follow-up exercise" list (StatefulSet vs Deployment, Ingress, resource limits, HPA) are in `k8s/README.md`.
* **Verified live on minikube: `postgres` and `agent` both reached `1/1 Running`**, with two real bugs found and fixed in the process:
  1. `ollama/ollama` has no explicit tag, so Kubernetes defaults `imagePullPolicy` to `Always` for it — which ignores an image already loaded into the node via `minikube image load` and just keeps retrying a registry pull. Fixed by pinning `:latest` explicitly and setting `imagePullPolicy: IfNotPresent`. Added the same explicit policy to `postgres.yaml` too, on principle (don't rely on implicit tag-based defaults).
  2. Both `postgres.yaml` and `ollama.yaml` use `strategy: type: Recreate` (required — a second pod can't share a ReadWriteOnce PVC). This means a stuck-Terminating old pod (which happened repeatedly with ollama — a pod that never got a running container still took minutes to clear) **fully blocks the new pod from ever being created**, unlike `RollingUpdate`. When this happens: `kubectl delete pod <name> --grace-period=0 --force` to unblock it.
* **`ollama` pod itself was NOT confirmed reaching Running this session** — got stuck indefinitely in `ContainerCreating` with zero container-process or event progress (confirmed via `crictl ps -a` / `crictl events` showing nothing), across multiple fresh attempts, after the image was confirmed correctly cached on the node (`crictl images` showed it present, 3.74GB). Also tried freeing host resources (stopping docker-compose's own `ollama` container, which was running the same model concurrently) — no improvement. This reads as host-level resource contention or a minikube/Docker Desktop instability on this specific machine, not a manifest bug — the manifest is structurally identical to the working `postgres.yaml`/`agent.yaml` pattern. **Next session: retry `kubectl get pods -n devops-agent` fresh; if still stuck, try `minikube delete && minikube start` for a fully clean node** rather than reusing the session that had the trouble.
* **Docker Desktop itself hung completely mid-session** (daemon unresponsive, `docker ps` timing out, `dockerd` process not even running inside the `docker-desktop` WSL2 distro) — likely from running the docker-compose stack and the minikube cluster's duplicate copies of the same heavy services (Ollama alone needs several GB) simultaneously. Recovered via `wsl --shutdown` + relaunching Docker Desktop. If this recurs, that's the fix — a plain "kill the GUI process and reopen" was NOT enough the first time it happened, only the full WSL shutdown worked.

**Not yet done:** nothing on the original 5-phase roadmap — all complete. Kubernetes is a bonus/learning addition on top, not part of the original scope, and the `ollama` pod's live-running state is the one open item there.
